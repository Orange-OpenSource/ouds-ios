...
Or if the [Codex (OpenAI product)](https://chatgpt.com/codex) product has been used to make some pull request reviews or to work on issues, using the GitHub Connector (see [here for LLM for Codex cloud details](https://openai.com/fr-FR/index/introducing-codex/)):
...
The main things to note are the LLM in use and the products bebibehindnd with, if relevant, company name to have no doubt.
There is no legal status yet about the ownership of the source code generated by AI, so use of *Reviewed-by* or *Co-authored-by* fields is not relevant.
