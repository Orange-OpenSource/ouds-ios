//
// Software Name: OUDS iOS
// SPDX-FileCopyrightText: Copyright (c) Orange SA
// SPDX-License-Identifier: MIT
//
// This software is distributed under the MIT license,
// the text of which is available at https://opensource.org/license/MIT/
// or see the "LICENSE" file for more details.
//
// Authors: See CONTRIBUTORS.txt
// Software description: A SwiftUI components library with code examples for Orange Unified Design System
//

import Foundation
import OUDSTokensComponent
import OUDSTokensRaw
import OUDSTokensSemantic

// 🧬 Tokens version: 2.7.0 (Orange brand)
// 🤖 Generated by Tokenator

extension OrangeThemeTextInputComponentTokensProvider: TextInputComponentTokens {
    /// Has for value token borders.radiusDefault (= 0)
    @objc open var borderRadiusDefault: BorderRadiusSemanticToken { borders.radiusDefault }
    /// Has for value token borders.radiusMedium (= 8)
    @objc open var borderRadiusRounded: BorderRadiusSemanticToken { borders.radiusMedium }
    /// Has for value token borders.widthDefault (= 1)
    @objc open var borderWidthDefault: BorderWidthSemanticToken { borders.widthDefault }
    /// Has for value token borders.widthMedium (= 2)
    @objc open var borderWidthFocus: BorderWidthSemanticToken { borders.widthMedium }
    /// Has for value token colors.borderDefault (= light: "#00000033" / dark: "#FFFFFF33")
    @objc open var colorBorderEnabled: MultipleColorSemanticToken { colors.borderDefault }
    /// Has for value token colors.borderEmphasized (= light: "#000000FF" / dark: "#EEEEEEFF")
    @objc open var colorBorderFocus: MultipleColorSemanticToken { colors.borderEmphasized }
    /// Has for value token colors.actionHover (= light: "#000000AD" / dark: "#FFFFFFA3")
    @objc open var colorBorderHover: MultipleColorSemanticToken { colors.actionHover }
    /// Has for value token colors.borderEmphasized (= light: "#000000FF" / dark: "#EEEEEEFF")
    @objc open var colorBorderLoading: MultipleColorSemanticToken { colors.borderEmphasized }
    /// Has for value token sizes.iconWithLabelLargeSizeXsmall (= 20)
    @objc open var sizeCountrySelectorFlagHeight: SizeSemanticToken { sizes.iconWithLabelLargeSizeXsmall }
    /// Has for value token DimensionRawTokens._600 (= 48)
    @objc open var sizeLabelMaxHeight: SizeSemanticToken { DimensionRawTokens._600 }
    /// Has for value token sizes.iconWithLabelLargeSizeSmall (= 24)
    @objc open var sizeLeadingIcon: SizeSemanticToken { sizes.iconWithLabelLargeSizeSmall }
    /// Has for value token DimensionRawTokens._4000 (= 480)
    @objc open var sizeMaxWidth: SizeSemanticToken { DimensionRawTokens._4000 }
    /// Has for value token DimensionRawTokens._750 (= 60)
    @objc open var sizeMinHeight: SizeSemanticToken { DimensionRawTokens._750 }
    /// Has for value token DimensionRawTokens._2500 (= 240)
    @objc open var sizeMinWidth: SizeSemanticToken { DimensionRawTokens._2500 }
    /// Has for value token dimensions._4xsmall (= 20)
    @objc open var sizeVerticalDividerHeight: SizeSemanticToken { dimensions._4xsmall }
    /// Has for value token {ouds1f4a0_columnGapMedium (= 12)
    @objc open var spaceColumnGapDefault: SpaceSemanticToken { {ouds1f4a0_columnGapMedium }
    /// Has for value token {ouds1f4a0_columnGapXsmall (= 6)
    @objc open var spaceColumnGapInlineText: SpaceSemanticToken { {ouds1f4a0_columnGapXsmall }
    /// Has for value token {ouds1f4a0_columnGap2xsmall (= 4)
    @objc open var spaceColumnGapLabelAsterisk: SpaceSemanticToken { {ouds1f4a0_columnGap2xsmall }
    /// Has for value token DimensionRawTokens.outOfSystem75 (= 3)
    @objc open var spaceColumnGapLabelSmallAsterisk: SpaceSemanticToken { DimensionRawTokens.outOfSystem75 }
    /// Has for value token {ouds1f4a0_columnGapXsmall (= 6)
    @objc open var spaceColumnGapTrailingErrorAction: SpaceSemanticToken { {ouds1f4a0_columnGapXsmall }
    /// Has for value token {ouds1f4a0_paddingBlock2xsmall (= 6)
    @objc open var spacePaddingBlockDefault: SpaceSemanticToken { {ouds1f4a0_paddingBlock2xsmall }
    /// Has for value token {ouds1f4a0_paddingBlockXsmall (= 8)
    @objc open var spacePaddingBlockTopHelperText: SpaceSemanticToken { {ouds1f4a0_paddingBlockXsmall }
    /// Has for value token {ouds1f4a0_paddingInlineXsmall (= 8)
    @objc open var spacePaddingInlineCountrySelectorEnd: SpaceSemanticToken { {ouds1f4a0_paddingInlineXsmall }
    /// Has for value token {ouds1f4a0_paddingInlineSmall (= 10)
    @objc open var spacePaddingInlineCountrySelectorStart: SpaceSemanticToken { {ouds1f4a0_paddingInlineSmall }
    /// Has for value token {ouds1f4a0_paddingInlineLarge (= 16)
    @objc open var spacePaddingInlineDefault: SpaceSemanticToken { {ouds1f4a0_paddingInlineLarge }
    /// Has for value token {ouds1f4a0_paddingInline2xsmall (= 6)
    @objc open var spacePaddingInlineTrailingAction: SpaceSemanticToken { {ouds1f4a0_paddingInline2xsmall }
    /// Has for value token {ouds1f4a0_rowGapNone (= 0)
    @objc open var spaceRowGapLabelInput: SpaceSemanticToken { {ouds1f4a0_rowGapNone }
}
