//
// Software Name: OUDS iOS
// SPDX-FileCopyrightText: Copyright (c) Orange SA
// SPDX-License-Identifier: MIT
//
// This software is distributed under the MIT license,
// the text of which is available at https://opensource.org/license/MIT/
// or see the "LICENSE" file for more details.
//
// Authors: See CONTRIBUTORS.txt
// Software description: A SwiftUI components library with code examples for Orange Unified Design System
//

import OUDSFoundations
import OUDSThemesContract

/// A class which wraps all **component  tokens of toast** for *toast* objects like `OUDSToast`.
/// Contains also references to semantic tokens providers so as to be able to use them to define the component tokens.
/// This provider should be integrated as a `AllToastComponentTokensProvider` implementation inside `OUDSTheme` so as to provide
/// all tokens to the users. It helps users to override some of the tokens and assign them to an `OUDSTheme` implementation to use.
/// Custom themes can use subclass of ``OrangeThemeToastComponentTokensProvider`` and apply the provider they need.
/// It implements also the protocol `ToastComponentTokens` so as to expose the component tokens for *toast* through any `OUDSTheme`.
/// *Toast* components tokens are defined with semantic tokens of borders (`AllBorderSemanticTokensProvider`),
/// dimensions (`AllDimensionSemanticTokensProvider`) and sizes (from `AllSizeSemanticTokensProvider`).
///
/// ```swift
///     // Define your own provider for toast component tokens
///     // by inheriting from existing provider
///     class CustomToastComponentTokensProvider: OrangeThemeToastComponentTokensProvider {
///
///         // Then override the toast component tokens you want.
///
///         override var sizeAssetSmall: SizeSemanticToken { DimensionRawTokens._100 }
///
///         override var borderRadiusMedia: BorderRadiusSemanticToken { BorderRawTokens.borderRadius0 }
///
///         // ...
///     }
///
///     // Or define your own provider from scratch
///     class CustomToastComponentTokensProvider: ToastComponentTokens {
///
///         // And implement hundreds of tokens.
///         // You are allowed to give semantic tokens providers if you want to define values.
///     }
/// ```
///
/// Then, you can give this `CustomToastComponentTokensProvider` to your own theme implementation:
///
/// ```swift
/// class LocalTheme: OrangeTheme {
///
///     override init() {
///         super.init(toast: CustomToastComponentTokensProvider())
///     }
/// }
/// ```
///
/// or to an already existing theme for example:
///
/// ```swift
///     OrangeTheme(toast: CustomToastComponentTokensProvider())
/// ```
///
/// It is also possible to use your own semantic tokens providers for this component tokens providers:
///
/// ```swift
///     // Uses by default here:
///     // - OrangeThemeBorderSemanticTokensProvider for borders
///     // - OrangeThemeDimensionSemanticTokensProvider for dimensions
///     // - OrangeThemeSizeSemanticTokensProvider for sizes
///     let toastComponentTokensProvider = OrangeThemeToastComponentTokensProvider()
///
///     // Or use your own border, dimension and size semantic tokens providers
///     let toastComponentTokensProvider = OrangeThemeToastComponentTokensProvider(
///                     borders: CustomBorderSemanticTokensProvider(),
///                     dimensions: CustomDimensionSemanticTokensProvider(),
///                     sizes: CustomSizeSemanticTokensProvider())
/// ```
///
/// - Since: 3.0.0
open class OrangeThemeToastComponentTokensProvider: AllToastComponentTokensProvider {

    /// Provider of borders semantic tokens to use for toast borders
    public let borders: AllBorderSemanticTokensProvider

    /// Provider of _dimensions_ semantic tokens to use for sizes as the Swift package exposes "closed" tokens of Figma
    public let dimensions: AllDimensionSemanticTokensProvider

    /// Provider of sizes semantic tokens to use for toast sizes
    public let sizes: AllSizeSemanticTokensProvider

    #if DEBUG
    nonisolated(unsafe) private static var instanceCount: Int = 0
    #endif
    /// Defines a provider of component tokens dedicated to `OUDSToast`
    /// - Parameters:
    ///    - borders: Provider for border semantic tokens, if nil, a default one will be used (``OrangeThemeBorderSemanticTokensProvider``)
    ///    - dimensions: Provider for dimension semantic tokens, if nil, default one will be used ( ``OrangeThemeDimensionSemanticTokensProvider``)
    ///    - sizes: Provider for size semantic tokens, if nil, default one will be used ( ``OrangeThemeSizeSemanticTokensProvider``)
    public init(borders: AllBorderSemanticTokensProvider? = nil,
                dimensions: AllDimensionSemanticTokensProvider? = nil,
                sizes: AllSizeSemanticTokensProvider? = nil)
    {
        OL.debug("Init of OrangeThemeToastComponentTokensProvider")
        self.borders = (borders ?? OrangeThemeBorderSemanticTokensProvider())
        self.dimensions = (dimensions ?? OrangeThemeDimensionSemanticTokensProvider())
        self.sizes = (sizes ?? OrangeThemeSizeSemanticTokensProvider(dimensions: self.dimensions))
        #if DEBUG
        Self.instanceCount++
        checkInstances(count: Self.instanceCount, for: "OrangeThemeToastComponentTokensProvider")
        #endif
    }

    deinit {
        #if DEBUG
        Self.instanceCount--
        #endif
    }

    // ଘ( ･ω･)_/ﾟ･:*:･｡☆
    // Note: So as to help the integration of generated code produced by the tokenator
    // the implementation of ToastComponentTokens is not here but in Core/Themes/Orange/Values/ComponentTokens/OrangeTheme+ToastComponentTokens.swift
    // This declaration of OrangeThemeToastComponentTokensProvider is here also to allow to write documentation.
}
