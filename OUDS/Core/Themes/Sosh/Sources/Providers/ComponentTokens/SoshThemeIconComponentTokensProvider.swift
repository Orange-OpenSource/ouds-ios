...
///
/// - Since: 0.20.0
final class SoshThemeIconComponentTokensProvider: AllIconComponentTokensProvider {
    ...
    init(colors: AllColorSemanticTokensProvider? = nil) {
        OL.debug("Init of SoshThemeIconComponentTokensProvider")
        self.colors = (colors ?? SoshThemeColorSemanticTokensProvider())
        #if DEBUG
        Self.instanceCount++
        checkInstances(count: Self.instanceCount, for: "SoshThemeIconComponentTokensProvider")
        #endif
    }
