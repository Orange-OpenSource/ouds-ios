...
extension OrangeInverseThemeColorModeSemanticTokensProvider: ColorModeMultipleSemanticTokens {
    ...
    public func toColor(from mode: MultipleColorModeSemanticTokens) -> MultipleColorSemanticTokens {
        guard !mode.hasUndefinedValue() else {
            OL.fatal("Trying to use color mode '\(mode.name)' with undefined value / forbidden color value for OrangeInverse Theme")
        }
        ...
        OL.fatal("The color mode '\(mode.name)' for OrangeInverse Theme is not managed and must be.")
    }
