...
/// Tests crash scenarios for `OrangeCompactTheme` color mode handling using Swift Testing's `#expect(exitsWith:)`.
...
struct OrangeCompactThemeColorModeCrashTests {

    /// Verify that `OrangeCompactTheme` crashes when trying to use a color mode token with undefined value.
    /// Undefined values (⛔️) are forbidden and should trigger a fatal error.
    @Test
    func orangeCompactThemeCrashesWithUndefinedColorModeValue() async {
        await #expect(processExitsWith: .failure) {
            let provider = OrangeCompactThemeColorModeSemanticTokensProvider()
    ...
    /// Verify that `OrangeCompactTheme` crashes when trying to use an unmanaged color mode token.
    /// All color mode tokens must be explicitly managed in the theme's `toColor(from:)` method.
    @Test
    func orangeCompactThemeCrashesWithUnmanagedColorModeToken() async {
        await #expect(processExitsWith: .failure) {
            let provider = OrangeCompactThemeColorModeSemanticTokensProvider()
