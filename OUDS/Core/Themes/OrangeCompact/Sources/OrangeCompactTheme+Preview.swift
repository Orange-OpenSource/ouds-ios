...
extension View {
    ...
    /// Wraps `self` `View` inside an `OUDSThemeableView` applying the ``OrangeCompactTheme``.
    ...
    ///     #Preview {
    ///        SampleView().orangeCompactPreview()
    ///     }
    /// ```
    ///
    /// ```swift
    ...
    ///         var body: some View {
    ///            yourContentView.orangeCompactPreview()
    ///         }
    ///     }
    ///
    ///     // In their side:
    ...
    @ViewBuilder
    public func orangeCompactPreview() -> some View {
        #if DEBUG
