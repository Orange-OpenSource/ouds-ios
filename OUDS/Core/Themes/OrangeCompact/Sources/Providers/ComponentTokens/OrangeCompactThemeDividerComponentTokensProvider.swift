...
///
/// - Since: 0.17.0
final class OrangeCompactThemeDividerComponentTokensProvider: AllDividerComponentTokensProvider {
    ...
    /// - Parameter borders: Provider for border semantic tokens. If nil, a default one will be used (``OrangeCompactThemeBorderSemanticTokensProvider``)
    init(borders: AllBorderSemanticTokensProvider? = nil) {
        OL.debug("Init of OrangeCompactThemeDividerComponentTokensProvider")
        self.borders = (borders ?? OrangeCompactThemeBorderSemanticTokensProvider())
        #if DEBUG
        Self.instanceCount++
        checkInstances(count: Self.instanceCount, for: "OrangeCompactThemeDividerComponentTokensProvider")
        #endif
    }
