...
public struct OOUDSButton: View {
    ...
    public init(hierarchy: Hierarchy, icon: Image, text: String) {
        self.type = .textAndIcon(text: text, icon: icon)
        self.hierarchy = hierarchy
    }
    public init(hierarchy: Hierarchy, icon: Image) {
        self.type = .icon(icon)
        self.hierarchy = hierarchy
    }

    public init(hierarchy: Hierarchy, text: String) {
        self.type = .text(text)
        self.hierarchy = hierarchy
    }
...
private struct TextForButton: View {
    ...
    var body: some View {
        Text(text).typeLabelStrongLarge(theme)
    }
}
